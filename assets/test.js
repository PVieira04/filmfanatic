 ...
 */
const generateQuestion = async (films) => {
...
const generateRandomQuestionNumber = async () => {
    ...
    while (true) {
        ...
        let idNumber = await fetchFilmArray().then(array => Math.floor(Math.random() * array.length) + 1);
        console.log(`THIS is the id number: ${idNumber}`);
        //Math.floor(Math.random() * films.length) + 1;
